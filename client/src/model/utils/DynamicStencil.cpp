...
void DynamicStencil::init(Renderer& ren, const int screenW, const int screenH, const float circleRadius, const float fovAngleDegrees, const uint8_t stencilAlpha, const float fovVisibleDistance) {
    screenWidth = screenW;
    screenHeight = screenH;
    const int stencilW = screenW * 2;
    const int stencilH = screenH * 2;
    stencil = std::make_shared<Texture>(ren, SDL_PIXELFORMAT_RGBA8888, SDL_TEXTUREACCESS_TARGET, stencilW, stencilH);
...
//     stencil = std::make_shared<Texture>(ren, SDL_PIXELFORMAT_RGBA8888, SDL_TEXTUREACCESS_TARGET, screenWidth, screenHeight);
//     stencil->SetBlendMode(SDL_BLENDMODE_BLEND).SetAlphaMod(stencilAlpha);
//     cirRadius = circleRadius;
//     fovAngle = fovAngleDegrees;
//     fovRadius = fovVisibleDistance;
...
void DynamicStencil::drawCircle(const Renderer& ren, const float cx, const float cy) {
    ...
    circleVertices.push_back(SDL_Vertex{
        SDL_FPoint{cx, cy},
        SDL_Color{0, 0, 0, 0},
        SDL_FPoint{}
    });
...
void DynamicStencil::drawFOVTriangle(const Renderer& ren, const float cx, const float cy) {
    float radius = (fovRadius != 0) ? fovRadius : static_cast<float>(std::max(screenWidth, screenHeight));
    float angleRad = 0.0f;  // Facing right
    float halfFOV = (fovAngle / 2.0f) * (M_PI / 180.0f);

    SDL_FPoint a = {cx, cy};
    SDL_FPoint b = {
        cx + radius * std::cos(angleRad - halfFOV),
    ...
    SDL_Vertex triangle[3] = {
        { a, SDL_Color{0, 0, 0, 0}, SDL_FPoint{} },
...
std::shared_ptr<Texture> DynamicStencil::getStencil() {
    return stencil;
}

FOVInfo DynamicStencil::getFOVInfo() {
    return {screenWidth, screenHeight, fovAngle, fovRadius};
}
